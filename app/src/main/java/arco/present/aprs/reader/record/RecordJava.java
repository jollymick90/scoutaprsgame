...
import static arco.present.aprs.reader.common.HideSickJava.outputFile;

public class RecordJava {
    private final Context ctx;
    private RecordWaveTask recordTask = null;

    public RecordJava(Context ctx) {
        this.ctx = ctx;
        recordTask = new RecordWaveTask(this.ctx);
    }

    public void launchTask(String file) {
        switch (recordTask.getStatus()) {
            case RUNNING:
                Toast.makeText(this.ctx, "Task already running...", Toast.LENGTH_SHORT).show();
                return;
            case FINISHED:
                recordTask = new RecordWaveTask(this.ctx);
                break;
